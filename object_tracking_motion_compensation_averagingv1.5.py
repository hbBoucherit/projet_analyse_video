    ...
    while (cap.isOpened()):
        ...
        if tracking or frame_number in bboxes : 
            if not tracking : 
                ...
                confidences = candidate_bboxes_predictions[:, labelIndex]
                avg_bboxes = np.average(candidate_bboxes, weights=np.power(confidences, 10), axis=0)
                # Update current box
