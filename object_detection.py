...
aspect_ratios = [[1,1], [0.5,0.5], [1.5, 1.5]]
steps = [0.5, 1, 1.5]
  ...
  frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    ...
    if tracking or frame_number in bboxes : 
        if not tracking : 
            ...
            candidate_bboxes = get_candidate_boxes(current_box, steps, aspect_ratios, fw, fh)
            candidate_bboxes_patches = get_candidate_patches(candidate_bboxes, frame)
            ...
            predictions[frame_number] = best_bbox, cv2.COLOR_BGR2RGB)

    if tracking or frame_number
    ...
    frame_numbermaxindex = int(np.argmax(prediction0))
...
class_names = {'Bowl': 0, 'CanOfCocaCola' : 1, 'MilkBottle' : 2, 'Rice' : 3, 'Sugar' : 4}

def read_annotation_file(file_path):
    # open txt file of boxes 

    class_name = str(file_path).split('Place')[0]
    label_index = class_names.get(class_name)
    ...
    with open(file_path, 'r') as f:
        lines = f.readlines()
        boxes = [[int(x) for x in row.split()] for row in lines]
...
def clip_boxes(boxes, im_width, im_height):
    ...
    cliped_boxes[:,0] =  np.clip(xyxy_boxes[:, 0], min=0, max=im_width-1)  # x
    cliped_boxes[:,1] =  np.clip(xyxy_boxes[:, 1], min=0, max=im_height-1) # y
    cliped_boxes[:,2] =  np.clip(xyxy_boxes[:, 2], min=0, max=im_width-1)  # x + w
    cliped_boxes[:,3] =  np.clip(xyxy_boxes[:, 3], min=0, max=im_height-1) # y + h
